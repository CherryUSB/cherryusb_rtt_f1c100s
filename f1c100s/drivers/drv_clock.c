...
rt_err_t cpu_set_pll_clk(int clk)
{
    ...
    CCU->pll_cpu_ctrl &= ~((unsigned int) 0x1 << 31);
    //PLL1 rate = ((24000000 * n * k) >> 0) / m   (p is ignored)
    CCU->pll_cpu_ctrl = ((unsigned int) 0x1 << 31) | (m << 0) | (k << 4) | (n << 8) | (p << 16);
...
rt_err_t audio_set_pll_clk(int clk)
{
    ...
    if (clk == 0)
    {
        CCU->pll_audio_ctrl &= ~((unsigned int) 0x1 << 31);
        return RT_EOK;
    }
    ...
    m = (m - 1) & 0x1f;
    //clk = (24 * n * 2) / m
    CCU->pll_audio_ctrl = ((unsigned int) 0x1 << 31) | (0x0 << 24) | (n << 8) | m;
...
rt_err_t video_set_pll_clk(int clk)
{
    ...
    if (clk == 0)
    {
        CCU->pll_video_ctrl &= ~((unsigned int) 0x1 << 31);
        return RT_EOK;
    }
    ...
    m = (m - 1) & 0xf;
    //speed = (24*n)/m
    CCU->pll_video_ctrl = ((unsigned int) 0x1 << 31) | (0x0 << 30) | (0x1 << 24) | (n << 8) | m;
...
rt_err_t ve_set_pll_clk(int clk)
{
    ...
    if (clk == 0)
    {
        CCU->pll_ve_ctrl &= ~((unsigned int) 0x1 << 31);
        return RT_EOK;
    }
    ...
    m = (m - 1) & 0xf;
    //clk = (24 * n) / m
    CCU->pll_ve_ctrl = ((unsigned int) 0x1 << 31) | (0x1 << 24) | (n << 8) | m;
...
rt_err_t periph_set_pll_clk(int clk)
{
    ...
    CCU->pll_periph_ctrl &= ~((unsigned int) 0x1 << 31);
    n = (n - 1) & 0x1f;
    k = (k - 1) & 0x3;
    //clk = 24 * n *k
    CCU->pll_periph_ctrl = ((unsigned int) 0x1 << 31) | (0x1 << 18) | (n << 8) | (k << 4) || (0x1);
...
rt_err_t mmc_set_clk(enum mmc_clk_id clk_id, int hz)
{
    ...
    *mmc_clk = ((unsigned int) 0x1 << 31) | pll | (sclk_dly << 20) | \
...
rt_err_t debe_set_clk(int clk)
{
    ...
    _div = v_clk / clk;
    if ((_div > 15) || ((clk * _div) != v_clk))
    {
        return RT_ERROR;
    }
    
    CCU->be_clk = ((unsigned int) 0x1 << 31) | (0x00 << 24) | ((_div - 1) & 0xf);
...
rt_err_t defe_set_clk(int clk)
{
    ...
    CCU->fe_clk = ((unsigned int) 0x1 << 31) | (0x00 << 24) | ((_div - 1) & 0xf);
...
{
    if (clk == TCON_VIDEO_X1)
    {
        CCU->tcon_clk = ((unsigned int)0x1 << 31) | (0x00 << 24);
    }
    else if (clk == TCON_VIDEO_X2)
    {
        CCU->tcon_clk = ((unsigned int) 0x1 << 31) | (0x02 << 24);
    }
    else
    {
        CCU->tcon_clk = 0x00;
...
int debe_get_clk(void)
{
    ...
    if (((temp >> 24) & 0x7) == 0)
    {
        return video_get_pll_clk() / ((temp & 0xf) + 1);
    }
    else if (((temp >> 24) & 0x7) == 2)
    {
        return periph_get_pll_clk() / ((temp & 0xf) + 1);
    }
    else
    {
        return 0;
