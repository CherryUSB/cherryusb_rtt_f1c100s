...
void rt_low_level_init(void)
{
    ...
    *addr = 0x1ff;
    ...
    addr = (unsigned int *)(0x01c20000 + 0x028);
    *addr |= ((unsigned int) 0x1 << 31);
    while (!(*addr & ((unsigned int) 0x1 << 28)));

    addr = (unsigned int *)(0x01c20000 + 0x054);
    *addr = (0x0 << 16) | (0x3 << 12) | (0x0 << 8) | (0x2 << 6) | (0x0 << 4);

    //reset all device
    addr = (unsigned int *)(0x01c20000 + 0x02c0);
    *addr = 0x4000;
    addr = (unsigned int *)(0x01c20000 + 0x02c4);
    *addr = 0x00;
    addr = (unsigned int *)(0x01c20000 + 0x02d0);
    *addr = 0x00;
